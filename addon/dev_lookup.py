...
# NHK 1998 audio collection: an index DB (built by dev_nhk1998_index.py) keyed
# by headword, with the mp3s themselves at _NHK1998_AUDIO_DIR. Audio-only — no
# pitch data — so it's a standalone source that attaches audio without writing
# a pitch line. A candidate's entry_id is "<headword><SEP><type>" (one entry
# per headword+type, collapsing every accent variant of that type).
_NHK1998_DB_PATH = os.path.join(
    os.path.dirname(__file__), "dictionary", "nhk1998", "nhk1998.db"
)
_NHK1998_AUDIO_DIR = "/Users/matt/Documents/Audio/nhk1998"
_NHK1998_TYPES = ('yomi', 'jyoshi', 'reibun')
_NHK1998_SEP = '\x1f'  # unit separator — never appears in a headword
...
# Match an audio tag generated by this lookup (daijisen.db / nhk.db). Used to
# strip prior-run audio from Word Audio so a fresh selection replaces stale
# files instead of piling on top. Non-lookup audio (manual, other addons) is
# preserved because it lacks the _NHK / _DAIJISEN2 / _NHK1998 suffix.
_LOOKUP_AUDIO_TAG_RE = re.compile(
    r'\[audio:[^\]]*_(NHK1998|NHK|DAIJISEN2)\.[^\]]+\]'
)
...
def _nhk1998_expand_keys(keys: set[str]) -> list[str]:
    """Expand match keys with hira/kata variants.

    nhk1998 kana headwords are written in hiragana (あいくち) while readings
    pulled from daijisen/NHK may be katakana, so try both directions for
    every key plus the raw surface form.
    """
    out: set[str] = set()
    for k in keys:
        if not k:
            continue
        out.add(k)
        out.add(_hira_to_kata(k))
        out.add(_katakana_to_hiragana(k))
    return sorted(out)


def _find_entries_nhk1998(keys: list[str], conn: sqlite3.Connection) -> list[str]:
    """Find nhk1998 candidates for the given headword keys.

    Returns "<headword><SEP><type>" ids — one per (headword, type) pair that
    has at least one file, ordered by headword then yomi/jyoshi/reibun.
    """
    if not keys:
        return []
    ...
    cur = conn.cursor()
    cur.execute(
        f"SELECT DISTINCT headword, type FROM files "
        f"WHERE headword IN ({placeholders})",
        keys,
    )
    order = {t: i for i, t in enumerate(_NHK1998_TYPES)}
    pairs = sorted(
        cur.fetchall(), key=lambda p: (p[0], order.get(p[1], 99))
    )
    return [f"{hw}{_NHK1998_SEP}{t}" for hw, t in pairs]


def _nhk1998_files(entry_id: str, conn: sqlite3.Connection) -> list[str]:
    """All filenames for an nhk1998 entry, in stable record order."""
    hw, _, typ = entry_id.partition(_NHK1998_SEP)
    cur = conn.cursor()
    cur.execute(
        "SELECT filename FROM files WHERE headword = ? AND type = ? "
        "ORDER BY position",
        (hw, typ),
    )
...
def _attach_nhk1998_audio(
    note, field_names: list[str], entry_id: str, filenames: list[str]
) -> None:
    """Copy nhk1998 mp3s into the Anki media dir and merge into Word Audio.

    Names are keyed by (headword, type, ordinal) with an _NHK1998 suffix so a
    re-lookup strips the prior selection via _LOOKUP_AUDIO_TAG_RE instead of
    piling on. Mirrors the daijisen/NHK copy path (ffmpeg re-encode to mp3,
    fall back to a raw copy as .aac when ffmpeg is unavailable).
    """
    if not filenames or "Word Audio" not in field_names:
        return
    hw, _, typ = entry_id.partition(_NHK1998_SEP)
    ...
    hw_clean = re.sub(r'[（）()]', '', hw)

    new_names: list[str] = []
    for i, orig in enumerate(filenames, 1):
        new_name = f"{hw_clean}_{typ}_{i}_NHK1998{ext}"
        src = os.path.join(_NHK1998_AUDIO_DIR, orig)
        dst = os.path.join(media_dir, new_name)
        if os.path.exists(src):
            # ffmpeg needs -y to skip the interactive overwrite prompt.
            if ffmpeg:
                subprocess.run(
                    [ffmpeg, '-y', '-i', src, '-q:a', '2', dst],
    ...
    existing_lines = (
        re.split(r'<br\s*/?>', note.fields[audio_idx])
        if note.fields[audio_idx] else []
    )
    kept = [
        l for l in existing_lines
        if l.strip() and not _LOOKUP_AUDIO_TAG_RE.search(l)
    ]
    new_tags = [f'[audio:{name}]' for name in new_names]
    combined = kept + [t for t in new_tags if t not in kept]
    note.fields[audio_idx] = '<br>'.join(combined).strip()
...
def _entry_display(entry_id: str, conn: sqlite3.Connection, source: str = 'daijisen') -> str:
    ...
    if source == 'nhk1998':
        hw, _, typ = entry_id.partition(_NHK1998_SEP)
        n = conn.execute(
            "SELECT COUNT(*) FROM files WHERE headword = ? AND type = ?",
            (hw, typ),
        ).fetchone()[0]
        return f"{hw} · [{typ}] ({n} file{'s' if n != 1 else ''})"
    ...
    if source == 'nhk':
        ...
        if row and row[1]:
            ...
            # Particle sibling: form is kanji+particle (e.g. '行き来を'), no
            # headline. Mirror the bare schema: surface kana+particle out front
            # and wrap the kanji+particle form in 【…】 in the label slot, so
            # every row uses the same "reading · 【kanji】 · [pitch]" shape.
            sibling_form = reading
            bare_id = entry_id.split('-')[0] + '-0000'
            if bare_id != entry_id and sibling_form.endswith(('を', 'に')):
                bare_row = cur.execute(
                    "SELECT form FROM entries WHERE id = ? LIMIT 1", (bare_id,)
                ).fetchone()
                if bare_row and bare_row[0]:
                    reading = bare_row[0] + sibling_form[-1]
                    label = f"【{sibling_form}】"
        rows = _get_nhk_pitch(entry_id, conn)
        pitch_list = list(dict.fromkeys(r[0] for r in rows)) if rows else []
    else:
    ...
    parts = [reading]
    if label:
        # No separator between the kana reading and the 【…】 kanji form —
        # standard Japanese-dictionary convention writes them adjacent.
        parts.append(label)
...
_SOURCE_PRIORITY = {'daijisen': 0, 'nhk': 1, 'nhk1998': 2}


def _reading_sort_key(entry_id: str, conn: sqlite3.Connection, source: str) -> str:
    """Normalized katakana sort key for picker grouping.

    NHK particle siblings (no headline, form ends in を/に) sort under their
    bare entry's reading so a 行き来を sibling clusters with its bare いきき
    (or ゆきき) entry, not with whatever 'ま…' the form happens to start with.
    """
    if source == 'nhk1998':
        hw, _, _typ = entry_id.partition(_NHK1998_SEP)
        return _hira_to_kata(_READING_STRIP_RE.sub('', hw))
    cur = conn.cursor()
    if source == 'nhk':
        row = cur.execute(
            "SELECT form, headline FROM entries WHERE id = ? LIMIT 1", (entry_id,)
        ).fetchone()
        form = row[0] if row and row[0] else ""
        if row and not row[1]:
            ...
            if bare_id != entry_id:
                ...
                if bare_row and bare_row[0]:
                    # Match the display schema: kana+particle, so a 行き来を
                    # sibling sorts just after its bare いきき entry.
                    form = bare_row[0] + form[-1] if form.endswith(('を', 'に')) else bare_row[0]
    else:
        row = cur.execute(
            "SELECT form FROM entries WHERE id = ? LIMIT 1", (entry_id,)
        ).fetchone()
        form = row[0] if row and row[0] else ""
    return _hira_to_kata(_READING_STRIP_RE.sub('', form))
...
def _show_entry_picker(
    ...
    sorted_candidates = sorted(
        candidates,
        key=lambda c: (
            _SOURCE_PRIORITY.get(c[0], 99),
            _reading_sort_key(c[1], conns[c[0]], c[0]),
            c[1],
        ),
    )

    lst = QListWidget()
    for src, eid in sorted_candidates:
        tag = {'daijisen': '[daijisen]', 'nhk': '[NHK]'}.get(src, '[NHK1998]')
...
def _lookup_note(editor: Editor):
    ...
    if (
        ...
        and not os.path.exists(_NHK_DB_PATH)
        and not os.path.exists(_NHK1998_DB_PATH)
    ):
    ...
    nhk1998_conn = (
        sqlite3.connect(f"file:{_NHK1998_DB_PATH}?mode=ro", uri=True)
        if os.path.exists(_NHK1998_DB_PATH) else None
    )
    ...
    # Override for the audio filename when the displayed Word-field reading
    # diverges from what the audio actually voices (NHK particle siblings).
    audio_reading: str | None = None

    try:
        candidates: list[tuple[str, str]] = []
        daij_ids: list[str] = []
        nhk_bare_ids: list[str] = []
        if daijisen_conn:
            daij_ids = _find_entries(word, daijisen_conn)
            candidates += [('daijisen', eid) for eid in daij_ids]
        if nhk_conn:
            nhk_bare_ids = _find_entries_nhk(word, nhk_conn)
            for bare_id in nhk_bare_ids:
                candidates.append(('nhk', bare_id))
                for sib in _find_nhk_particle_siblings(bare_id, nhk_conn):
                    candidates.append(('nhk', sib))
        if nhk1998_conn:
            # "Surface, then reading": match the bare Word against the
            # headword prefix, plus any kana readings the daijisen/NHK
            # lookups turned up for the same word.
            keys = {word}
            for eid in daij_ids:
                r = _get_reading(eid, daijisen_conn)
                if r:
                    keys.add(r[0])
            for bid in nhk_bare_ids:
                r = _get_reading(bid, nhk_conn)
                if r:
                    keys.add(r[0])
            candidates += [
                ('nhk1998', eid)
                for eid in _find_entries_nhk1998(
                    _nhk1998_expand_keys(keys), nhk1998_conn
                )
            ]
        ...
        if source == 'nhk1998':
            # Audio-only: attach every file for the picked (headword, type)
            # and leave the Word field untouched (no pitch data here).
            assert nhk1998_conn is not None
            _attach_nhk1998_audio(
                note, field_names, selected,
                _nhk1998_files(selected, nhk1998_conn),
            )
            if note.id:
                mw.col.update_note(note)
            editor.loadNoteKeepingFocus()
            hw, _, typ = selected.partition(_NHK1998_SEP)
            tooltip(f"{hw} → {typ} audio (nhk1998)")
            return
        ...
        if source == 'daijisen':
            ...
            if rows4:
                ...
                if e_headline:
                    ...
                    # +particle audio. NHK lists own-reading audios first per
                    # drops in position order, then cross-listed alternate-
                    # reading audios for the same kanji form. First-wins picks
                    # the audio that actually voices this entry's reading.
                    bare_id = selected.split('-')[0] + '-0000'
                    bare_rows = _get_nhk_pitch(bare_id, nhk_conn)
                    if bare_rows:
                        particle_audio_by_drops: dict[str, str] = {}
                        for (d, _s, a, _dv) in rows4:
                            if a and d not in particle_audio_by_drops:
                                particle_audio_by_drops[d] = a
                        ...
                        # File-naming overrides for the sibling case: the
                        # filename should reflect the +particle form the audio
                        # actually voices (e.g. 行き来を_いききを_2_NHK.mp3),
                        # while the Word field keeps the bare lemma above.
                        sib_row = cur.execute(
                            "SELECT form FROM entries WHERE id = ? LIMIT 1",
                            (selected,),
                        ).fetchone()
                        sib_form = sib_row[0] if sib_row else None
                        if sib_form and sib_form.endswith(('を', 'に')):
                            hyouki = sib_form
                            if reading_info and reading_info[0]:
                                audio_reading = reading_info[0] + sib_form[-1]
                    else:
                        pitch_rows = rows4
                        reading_info = _get_reading(selected, nhk_conn)
                        hyouki = None
    ...
    # Dedupe lines: pitch_rows often has duplicate (drops, devoiced) tuples
    # — NHK lists own-reading and cross-listed alternate-reading audios under
    # the same drops/devoiced, so each pitch line appears once per audio.
    note.fields[word_idx] = '<br>'.join(dict.fromkeys(lines))

    # Copy audio files and populate Word Audio field
    if audio_files and "Word Audio" in field_names:
        # Dedupe by drops only (not by (audio, drops)): NHK lists own-reading
        # audios first per drops, then cross-listed alternate-reading audios
        # for the same kanji form. Keeping only the first audio per drops
        # picks own-reading; the filename is keyed by drops too, so writing
        # the alternates would just overwrite the right file with the wrong
        # content under the same name.
        seen: set[str] = set()
        unique_pairs: list[tuple[str, str]] = []
        for (pitch_drop, _split, audio, _dv) in pitch_rows:
            if audio and str(pitch_drop) not in seen:
                seen.add(str(pitch_drop))
        ...
        r_clean = _strip_parens(audio_reading or clean_reading)
        new_names: list[str] = []
        for orig_audio, pitch in unique_pairs:
            ...
            dst = os.path.join(media_dir, new_name)
            if os.path.exists(src):
                # Overwrite any existing dst: the filename is keyed by
                # (kanji, reading, pitch), so a file from a previous version
                # of this addon (or a previous selection that hit the same
                # name) may hold content from a different source audio.
                # ffmpeg needs -y to skip the interactive overwrite prompt.
                if ffmpeg:
                    subprocess.run(
                        [ffmpeg, '-y', '-i', src, '-q:a', '2', dst],
        ...
        audio_idx = field_names.index("Word Audio")
        existing_lines = re.split(r'<br\s*/?>', note.fields[audio_idx]) if note.fields[audio_idx] else []
        kept = [l for l in existing_lines if l.strip() and not _LOOKUP_AUDIO_TAG_RE.search(l)]
        new_tags = [f'[audio:{name}]' for name in new_names]
        combined = kept + [t for t in new_tags if t not in kept]
        note.fields[audio_idx] = '<br>'.join(combined).strip()
